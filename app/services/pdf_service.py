...
class ReportPDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 8, f"Market Research Report — {self.company_name}", align="L")
        self.ln(4)
    ...
    def tag(self, label: str, color: tuple):
        ...
        self.cell(
            self.get_string_width(label) + 6, 5, label,
            fill=True, align="C",
        )
...
def generate_pdf(job: ResearchJob) -> bytes:
    ...
    pdf.cell(0, 12, f"{job.query}", new_x="LMARGIN", new_y="NEXT")
    ...
    for source in r.sources:
        ...
        pdf.cell(0, 4.5, f"• {title}", new_x="LMARGIN", new_y="NEXT")
